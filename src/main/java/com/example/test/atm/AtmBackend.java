...
public class AtmBackend {
    ...
    public int decreaseMoney (int money, int currency, int currencyAmount) {
        ...
        return money - currency * currencyAmount;
    }

    public int parseMoney(int money, int currency, int returnAmount, int amountInDatabase, String op){
        int amount = money / currency;
        if (amount != 0 && amountInDatabase >= amount){
            System.out.println("parsing " + currency);
            returnAmount = amount;
        }
        if (op.equals("-"))
        {
            return checkDatabase(returnAmount, amountInDatabase);
        }
    ...
    public void updateBalance(int money, String op){
        ...
        try {
            ...
            if (op.equals("+")) {
                ...
                twentyAmount = parseMoney(money, 20, twentyAmount, atm.getTwenty(), "-");
                atm.setTwenty(atm.getTwenty() - twentyAmount);
                money = decreaseMoney(money, 20, twentyAmount);
                System.out.println(money);
